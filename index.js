    ...
    //Option 2: multiple restaurants, sort by wait time
      ...
      var apiRequest = https.request(options, function (apiResponse) {
        ...
        apiResponse.on("end", function (chunk) {
          ...
          // Check if 0 results
          if (!restaurantRes) {
            responseText += "There are no restaurants in your area"
            res.json({
              message: responseText,
              zipCode: zip,
              requestedRestaurantName: tempName,
         ...
         .then(function (response) {
           ...
           //  if(responseText.localeCompare("")){ //TODO: Add variables
             if(responseText.localeCompare("")){ //TODO: Add variables to responsetext
               responseText +=  "You can call them at " + response.data.businesses[0].display_phone + ". They are currently using " + response.data.businesses[0].transactions
             }
             // Case where Visa API has results and we are adding to them
           //  else { //TODO: Add variables
             else { //TODO: Add variables to responsetext
               responseText += "You should go to " + response.data.businesses[0].name + " at " + response.data.businesses[0].location.displayAddress +". Their number is " + response.data.businesses[0].phone +". Try their webpage at " + response.data.businesses[0].url + ". They are currently using " + response.data.businesses[0].transactions
             }
             //responseText += restObj.businesses[0].display_address;
             //let responseText = req.body.queryResult.outputContexts.length;
